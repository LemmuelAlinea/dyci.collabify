...
function Attachment({ attachment, mine }: { attachment: MessageAttachment; mine: boolean }) {
  ...
  return (
    ...
    >
      ...
      <span className="shrink-0 text-[12px] opacity-60">{formatBytes(attachment.size_bytes)}</span>
    </button>
  )
}
      ...
      <div className={`flex min-w-0 max-w-[min(78%,520px)] flex-col ${mine ? 'items-end' : 'items-start'}`}>
        ...
        <div className={`flex items-start gap-1 ${mine ? 'flex-row-reverse' : ''}`}>
          ...
          >
            {message.pinned && (
              <p
                className={`mb-1.5 flex items-center gap-1 text-[12px] font-medium tracking-wide uppercase ${
            ...
            {editing ? (
              ...
              <>
                ...
                <p
                  className={`mt-1 text-[12px] ${mine && !poll ? 'text-white/55' : 'text-faint'}`}
                >
