...
export function ClassCodePill({ code, tone = 'quiet' }: { code: string; tone?: 'quiet' | 'loud' }) {
  return (
    <span
      className={`inline-flex items-center rounded-lg px-2 py-0.5 font-mono text-[11px] tracking-wide @min-[240px]:px-2.5 @min-[240px]:py-1 @min-[240px]:text-[12px] ${
        tone === 'loud'
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-2.5 @min-[240px]:gap-3.5">
        <span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-navy-600 font-display text-[12px] font-bold tracking-tight text-amber-400 @min-[240px]:h-12 @min-[240px]:w-12 @min-[240px]:rounded-xl @min-[240px]:text-[15px] dark:bg-navy-500">
          {cls.initial}
        </span>
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 truncate text-[11px] text-muted @min-[240px]:mt-1 @min-[240px]:text-[12.5px]">
            {classMeta(cls)}
          </p>
        </div>
        {archived && (
          <span className="shrink-0 rounded-full surface-sunken px-2 py-0.5 font-mono text-[9.5px] tracking-wider text-faint uppercase @min-[240px]:px-2.5 @min-[240px]:py-1">
            Archived
          </span>
        )}
      </div>
      ...
      {cls.professor && (
        <p className="mt-2 truncate text-[11.5px] text-muted @min-[240px]:mt-3 @min-[240px]:text-[13px]">
      ...
      <div className="mt-auto flex flex-wrap items-center justify-between gap-x-3 gap-y-1.5 border-t border-line pt-2.5 @min-[240px]:pt-3.5">
        ...
        <span className="flex items-center gap-1.5 text-[11.5px] text-muted @min-[240px]:text-[12.5px]">
